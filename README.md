...
* **`src/data/`**: Contém o arquivo `faperj-data.ts` que armazena **TODOS os dados estáticos** do projeto, garantindo a separação de código e dados (SoC).
...
* **Dados dos Gráficos:** As informações exibidas nos gráficos são provenientes do arquivo de dados estático (`src/data/faperj-data.ts`).
