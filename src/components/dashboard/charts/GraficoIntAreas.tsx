import React, { useMemo } from "react";
import ReactECharts from "echarts-for-react";
import { Card, Typography, Box, CircularProgress, Alert } from "@mui/material";
import useFaperjData from "@/hooks/useFaperjData";
import { IntAreasData } from "@/types/faperj";

const GraficoIntAreas: React.FC = () => {
  const { data, loading, error } = useFaperjData<IntAreasData>("int_areas");

  // 🔥 useMemo SEMPRE no topo, antes de qualquer return condicional
  const option = useMemo(() => {
    if (!data) return {};

    const colors = [
      "#5A8FDC",
      "#76B5C5",
      "#A3D1C6",
      "#F2C879",
      "#E8956F",
      "#C47AC0",
      "#7A6FF0",
      "#63C28F",
      "#C5C5C5",
      "#8BA3A8",
      "#A0A0A0",
    ];

    return {
      grid: {
        left: 180,
        right: 30,
        top: 50,
        bottom: 50,
      },

      tooltip: {
        trigger: "item",
        backgroundColor: "#fff",
        borderColor: "rgba(0,0,0,0.15)",
        borderWidth: 1,
        textStyle: { color: "#000" },
        borderRadius: 6,
        padding: 10,
        formatter: (p: any) => `
          <strong>${p.data.label}</strong><br/>
          Instituições: <strong>${p.data.value}</strong>
        `,
      },

      xAxis: {
        type: "value",
        axisLabel: { fontSize: 12 },
        splitLine: {
          lineStyle: { type: "dashed", color: "#ccc" },
        },
      },

      yAxis: {
        type: "category",
        data: data.map((i) => i.label),
        axisLabel: { fontSize: 12 },
      },

      series: [
        {
          type: "bar",
          barWidth: "45%",
          data: data.map((i, idx) => ({
            value: i.value,
            label: i.label,
            itemStyle: { color: colors[idx % colors.length] },
          })),
          label: {
            show: true,
            position: "right",
            formatter: "{c}",
            fontSize: 12,
            color: "#444",
          },
        },
      ],
    };
  }, [data]);

  // 🔥 Agora os retornos condicionais vêm DEPOIS dos hooks
  if (loading)
    return (
      <Box p={3} textAlign="center">
        <CircularProgress />
      </Box>
    );

  if (error) return <Alert severity="error">Erro ao carregar os dados.</Alert>;
  if (!data) return <Alert severity="warning">Dados indisponíveis.</Alert>;

  return (
    <Card
      sx={{
        p: 3,
        borderRadius: 3,
        boxShadow: 3,
        height: 430,
        display: "flex",
        flexDirection: "column",
      }}
    >
      <Typography
        variant="h6"
        fontWeight={700}
        color="#124b6c"
        sx={{ mb: 1, fontSize: "18px" }}
      >
        Instituições por Grande Área
      </Typography>

      <Box sx={{ width: "100%", height: "1px", backgroundColor: "rgba(0,0,0,0.1)", mb: 2 }} />

      <Box sx={{ flexGrow: 1 }}>
        <ReactECharts option={option} style={{ height: "100%", width: "100%" }} />
      </Box>

      <Typography
        variant="caption"
        sx={{
          mt: 1,
          color: "rgba(0,0,0,0.6)",
          fontStyle: "italic",
        }}
      >
        Fonte: Sistema de Bolsas e Auxílios – SBA / FAPERJ (Dados Internacionais)
      </Typography>
    </Card>
  );
};

export default GraficoIntAreas;
